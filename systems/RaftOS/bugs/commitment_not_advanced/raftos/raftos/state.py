...
#here we found an error, append_entries_response may not have request_id key
...
class Leader(BaseState):
    ...
    def update_commit_index(self):
        ...
        for index in range(self.log.commit_index + 1, self.log.last_log_index + 1):
            ...
            print("self.log[index]['term']  ", self.log[index]['term'])
            print("self.storage.term", self.storage.term)
...
class Follower(BaseState):
    ...
    def on_receive_request_vote(self, data):
        if self.storage.voted_for is None  and not data['type'].endswith('_response'):
            ...
            print("in receive function voted_for", self.storage.voted_for)
...
class State:
    ...
    def __init__(self, server):
        self.server = server
        self.id = self.masque_addr(self._get_id(server.host, server.port))
    ...
    # return masque addr
    def masque_addr(self, addr):
        if not config.net_address_map is None:
            masque_addr = config.net_address_map[addr]
        else :
            print("masque_addr is None")
        return masque_addr
