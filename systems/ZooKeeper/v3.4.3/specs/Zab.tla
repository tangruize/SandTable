...
UpdateRecorderZab(pc) == RecorderSetTransactionNum @@ RecorderSetMaxEpoch
...
NodeCrash(i) ==
        ...
        /\ \/ /\ IsLooking(i)
              /\ UNCHANGED <<varsLNoNet, zabState, connectInfo, learners,
                             forwarding, connecting, electing, ackldRecv>>
           \/ /\ IsFollower(i)
...
ConnectAndFollowerSendFOLLOWERINFO(i, j) ==
        ...
        /\ Net2Action(Send(j, i, [ mtype |-> FOLLOWERINFO, mzxid |-> <<acceptedEpoch[j], 0>> ]),
                      <<"ConnectAndFollowerSendFOLLOWERINFO", i, j>>)
...
SendSyncMsgs(i, j, lastSeenZxid, lastSeenIndex, mode, needRemoveHead) ==
        ...
        /\ LET lastCommittedIndex == IF zabState[i] = BROADCAST 
           ...
           IN /\ proposalMsgsLog' = proposalMsgsLog \union set_forChecking
              /\ \/ /\ needRemoveHead
                    /\ Net2Action(DiscardAndSendPackets(i, j, queue_toSend), pc)
                 \/ /\ ~needRemoveHead
                    /\ Net2Action(SendPackets(i, j, queue_toSend), pc)
        ...
        /\ LET msg == msgs[j][i][1]
           ...
           IN /\ infoOk
              /\ \/ \* electionFinished = true, jump ouf of waitForEpochAck. 
                    ...
                    /\ Net2Action(Discard(j, i), Append(pc, "ElectionFinished"))
                 \/ /\ ~ElectionFinished(i, sid_electing)
                    /\ \/ /\ zabState[i] = DISCOVERY
                          /\ UNCHANGED violatedInvariants
                       \/ /\ zabState[i] /= DISCOVERY
                    ...
                    /\ \/ /\ followerStateSummary.currentEpoch = -1
                          ...
                          /\ Net2Action(Discard(j, i), Append(pc, "currentEpoch=-1"))
                       \/ /\ followerStateSummary.currentEpoch > -1
                          /\ \/ \* normal follower 
                                /\ logOk
...
FollowerProcessSyncMessage(i, j) ==
        ...
        /\ LET msg == msgs[j][i][1]
           ...
           IN /\ infoOk
              ...
              /\ Net2Action(Discard(j, i), <<"FollowerProcessSyncMessage", i, j, msg.mtype>>)
