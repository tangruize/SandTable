...
def yield_trace(states):
    def compare(n, code_var_name, model_var_name, model_var_value, state_collect=False, sleep=0):
        if state_collect:
            yield ['intercept', n, 'state_collect']
        if sleep:
            yield ['sleep', str(sleep)]
        yield ['#', 'variable', model_var_name, n, str(model_var_value)]
        if not disable_compare:
            yield ['intercept', n, 'state_get', code_var_name]
            yield ['compare', 'variable']
    node_conv_dict = {'n1': 1, 'n2': 2, 'n3': 3, 'n4': 4, 'n5': 5}
    def node_conv(node):
        return node_conv_dict[node]
    def zxid_conv(zxid):
        a, b = zxid
        return (a << 32) + b
    def do_tick(n, is_compare=True):
        yield ['sleep', '500']  # sleep to wait the node processing
    fle_actions = {"ReceiveNotmsg", "NotmsgTimeout", "HandleNotmsg", "ZabTimeout", "WaitNewNotmsg"}
    ...
    for i, msg, election_msgs, cur_state in zip(jq_trace.input(states), jq_msgs.input(states), jq_election_msgs.input(states), states):
        states_counter += 1
        comment = i[0]
        if is_initing and comment[0] not in {'Init', 'ZabTimeout'}:
            is_initing = False
            yield ['sleep', '2000']
        ...
        if len(i) > 1:
            ...
            if cmd == 'conn_del_flush':  # NodeCrash
                ...
                # yield ['sleep', '3000']
                # yield ['partition', comment[1]]
                yield ['pre_close_client', comment[1]]
                yield ['recover', comment[1]]  # recover is reset the connections
                yield ['execute', comment[1], "#crash"]
                # yield ['execute', comment[1], "#interrupt"]
                yield ['sleep', '3000']
                yield ['close_client', comment[1]]
                ...
                elif comment[0] in {"FollowerProcessPROPOSALInSync", "FollowerProcessCOMMITInSync"}:
                    pass  # have been delivered above
                else:
                    yield from deliver(parameters[1], parameters[0], comment[0])
                # yield from deliver(parameters[1], parameters[0], comment[0])
                ...
                if not is_initing and comment[0] == "ZabTimeout":
                    ...
                    if node_start:
                        node_start = False
                        # clear the retransmission msgs (safe to clear according to the code)
                        dst = comment[1]
                        for src in nodes:
                            if src != dst:
                                yield ['clear', src, dst + ':3888']
                if comment[0] == 'NotmsgTimeout':
                    yield ['intercept', comment[1], 'inc_time_ms', '210']
                    yield ['sleep', '100']
                if not is_initing and comment[0] in fle_actions:
                    yield ['intercept', comment[1], 'state_collect']
                    yield ['sleep', '100']
                    # Sending Notification
                    yield from compare(comment[1], 'proposedLeader', 'currentVote.proposedLeader', node_conv(cur_state['currentVote'][comment[1]]['proposedLeader']))
                    yield from compare(comment[1], 'proposedZxid', 'currentVote.proposedZxid', zxid_conv(cur_state['currentVote'][comment[1]]['proposedZxid']))
                    yield from compare(comment[1], 'proposedEpoch', 'currentVote.proposedEpoch', cur_state['currentVote'][comment[1]]['proposedEpoch'])
                    yield from compare(comment[1], 'logicalclock', 'logicalClock', cur_state['logicalClock'][comment[1]])
                pass
            elif cmd == 'msg_batch_add_reply':  # recv msg and batch send msgs
                # yield ['deliver', parameters[1], parameters[0]]
                if comment[0] == "FollowerProcessNEWLEADER":
                    pass  # has been delivered.
                else:
                    yield from deliver(parameters[1], parameters[0], comment[0])
        else:
            if comment[0] == 'Init':
                # yield ['sleep', '2000']  # sleep 2 seconds to wait nodes init
                yield ['init', str(len(nodes.keys())), '8000']
                yield ['wait-init', str(len(nodes.keys())), '0', '8000']
            if comment[0] == 'WaitNewNotmsg' and comment[-1] == 'timeout':
                ...
                if comment[-2] == "LEADING":
                    ...
                    yield ['sleep', '500']
                yield from compare(comment[1], 'zabState', 'zabState', cur_state['zabState'][comment[1]].lower(), state_collect=True, sleep=100)

        if comment[0] == 'LeaderProcessFOLLOWERINFO':
            #  Follower sid:
            yield from compare(comment[1], 'follower_sid', 'follower_sid', node_conv(comment[2]), state_collect=True, sleep=100)
        elif comment[0] == 'FollowerProcessLEADERINFO' and comment[-1] == 'newEpoch bigger':
            # Follower.java#L107
            yield from compare(comment[1], 'zabState', 'zabState', 'synchronization', state_collect=True, sleep=100)
        elif comment[0] == 'FollowerProcessNEWLEADER':
            yield from compare(comment[1], 'msg_type', 'msg_type', 'NEWLEADER', state_collect=True, sleep=100)
        # elif comment[0] == 'LeaderSyncFollower':
        #     yield from compare(comment[1], 'sync_msg_type', 'sync_msg_type', comment[-1], state_collect=True, sleep=100)
            # yield from compare(comment[1], 'zxid', 'zxid', zxid_conv(cur_state['lastCommitted'][comment[1]]['zxid']))
        elif comment[0] == 'LeaderProcessACKLD':
            yield from compare(comment[1], 'msg_type', 'msg_type', 'NEWLEADER-ACK', state_collect=True, sleep=100)
        elif comment[0] == 'FollowerProcessUPTODATE':
            yield from compare(comment[1], 'msg_type', 'msg_type', 'UPTODATE', state_collect=True, sleep=100)

        # for j in nodes:
            #     yield from do_tick(j)
        if not is_initing and comment[0] != "ReceiveNotmsg" and show_status:
            yield ['status']
            for src in nodes:
