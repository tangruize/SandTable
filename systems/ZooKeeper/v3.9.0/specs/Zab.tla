...
net2 == INSTANCE FifoNetwork WITH FLUSH_DISCONN <- TRUE, NULL_MSG <- NONE,
        _msgs <- msgs, _netman <- recorder, _netcmd <- netcmd
...
vars == <<serverVars, electionVars, leaderVars, followerVars,
          verifyVars, msgVars, envVars, recorder, netcmd, historyVotes>> 
-----------------------------------------------------------------------------
...
\*                         /\ partition[j][i] = TRUE
HasPartitioned(i, j) == ~net2!NetIsConn(i, j)
-----------------------------------------------------------------------------
\* Check zxid state
...
RecorderIncPartition == RecorderIncHelper("nPartition")
RecorderIncRequests == RecorderIncHelper("nClientRequest")
...
RecorderSetTransactionNum == IF history /= history' THEN
                                    LET s == CHOOSE i \in Server:
                                        \A j \in Server: Len(history'[i]) >= Len(history'[j])
                                    IN ("nTransaction" :> Len(history'[s]))
                                 ELSE <<>>
...
RecorderSetMaxEpoch      == IF acceptedEpoch /= acceptedEpoch' THEN
                                    LET s == CHOOSE i \in Server:
                                        \A j \in Server: acceptedEpoch'[i] >= acceptedEpoch'[j]
                                    IN ("maxEpoch" :> acceptedEpoch'[s])
                                ELSE <<>>
\* RecorderSetRequests(pc)       == ("nClientRequest" :>
\*                                 IF pc[1] = "LeaderProcessRequest" THEN
\*                                     recorder["nClientRequest"] + 1
\*                                 ELSE recorder["nClientRequest"] )
\* RecorderSetRequests       == ("nClientRequest" :> recorder["nClientRequest"] + 1)
...
\*                           []   pc[1] = "PartitionStart"  -> RecorderIncPartition
\*                           []   pc[1] = "NodeCrash"       -> RecorderIncCrash 
\*                           []   OTHER                     -> <<>>

\* UpdateRecorderOrig(pc) == recorder' = RecorderSetPartition(pc)
\*                                   @@  RecorderSetFailure(pc)  @@ RecorderSetTransactionNum(pc)
\*                                   @@ RecorderSetMaxEpoch(pc)  \*@@ RecorderSetPc(pc) 
\*                                   @@ RecorderSetRequests(pc)  @@ recorder

RecorderClearZabLock == [ zabLock |-> NONE, zabLockArgs |-> NONE ]

RecorderClearFSyncLock == IF recorder.zabLock /= NONE /\ recorder.zabLock = "fsync"
                          THEN IF Len(msgs[recorder.zabLockArgs[2]][recorder.zabLockArgs[1]]) <= 1
                               THEN RecorderClearZabLock
                               ELSE <<>>
                          ELSE <<>>

UpdateRecorderZab(pc) == RecorderSetTransactionNum @@ RecorderSetMaxEpoch @@ RecorderClearFSyncLock

UpdateRecorderFLE(pc) == IF Len(pc) > 1 THEN Net1Args(pc[2]) ELSE <<>>

UpdateRecorder(pc) ==  UpdateRecorderFLE(pc) @@ UpdateRecorderZab(pc)

Net2NoAction(pc) == net2!NetUpdate(net2!NetNoActionArgCmd(UpdateRecorder(pc), pc))
Net2Action(updater, pc) == net2!NetUpdate2(net2!NetArg(UpdateRecorder(pc), updater), pc)
Net2ActionArg(updater, pc, arg) == net2!NetUpdate2(net2!NetArg(arg @@ UpdateRecorder(pc), updater), pc)
Net2ActionNoUpdate(updater, pc) == net2!NetUpdate2(updater, pc)
...
PreCheckParameterLimit(n, p) == CheckParameterHelper(n, p, LAMBDA i, j: i < j)
...
CheckWireMsgs       == CheckParameterLimit(recorder.n_wire,      "MaxWireMsgs")
...
CheckCrash == CheckParameterLimit(recorder.nCrash, "MaxCrashes")
PreCheckCrash == PreCheckParameterLimit(recorder.nCrash, "MaxCrashes")
CheckStateConstraints == CheckCrash /\ CheckTimeout /\ CheckTransactionNum /\ CheckEpoch /\ CheckWireMsgs
-----------------------------------------------------------------------------
...
\*                                                                  ELSE msgs[i][v]]]
Broadcast(i, m) == net2!NetBatchAddMsgDsts(i, forwarding[i] \ {i}, m)
...
\*                                                       THEN Append(msgs[i][v], m)
\*                                                       ELSE msgs[i][v]]]
DiscardAndBroadcast(i, j, m) == net2!NetReplyBatchAddMsgDsts(i, j, forwarding[i] \ {i}, m)
...
DiscardAndBroadcastLEADERINFO(i, j, m) ==
    LET new_connecting_quorum == {c \in connecting'[i]: c.connected = TRUE }
        new_sid_connecting == {c.sid: c \in new_connecting_quorum }
        dsts == (new_sid_connecting \intersect learners[i]) \ {i}
    IN net2!NetReplyBatchAddMsgDsts(i, j, dsts, m)
...
DiscardAndBroadcastUPTODATE(i, j, m) ==
    LET new_ackldRecv_quorum == {a \in ackldRecv'[i]: a.connected = TRUE }
        new_sid_ackldRecv == {a.sid: a \in new_ackldRecv_quorum}
        dsts == (new_sid_ackldRecv \intersect learners[i]) \ {i}
    IN net2!NetReplyBatchAddMsgDsts(i, j, dsts, m)
...
Reply(i, j, m) == net2!NetReplySrcDstMsg(i, j, m)

\* \* Shuffle input buffer.
\* Clean(i, j) == msgs' = [msgs EXCEPT ![j][i] = << >>, ![i][j] = << >>]
Clean(i) == net2!NetDisconnect({i})
CleanConn(i) ==
    LET clean1 == net1!NetDisconnect({i})
        clean2 == net2!NetDisconnect({i})
        n_wire == ("n_wire" :> clean1[1].n_wire + clean2[1].n_wire - recorder.n_wire)
    IN <<clean1[2], n_wire @@ clean2[1], clean2[2], clean2[3]>>
...
\*                                                               ELSE msgs[s][v] ] ]
\* CleanInputBufferInCluster(S) == net2!NetDisconnectOthers(S)
...
InitNet == LET args == [lock           |-> INITING,
                        ...
                        zabLockArgs    |-> NONE]
               pc == <<"Init">>
           IN  /\ net1!InitFifoNetworkAddNetman(Server, pc, args)
               /\ net2!InitFifoNetworkAddNetman(Server, pc, args)
...
Init == /\ InitServerVars
        ...
        \* /\ InitMsgVars
        \* /\ InitRecorder
        \* /\ fleRecorder = [ pc |-> <<"init">>, lock |-> INITING, init |-> Server]
-----------------------------------------------------------------------------
ZabTurnToLeading(i) ==
...
FleHanleMsg(i) ==
        /\ LET newState == state'[i]
           ...
           \/ /\ newState = FOLLOWING
              /\ ZabTurnToFollowing(i)
              /\ UNCHANGED <<learners, connecting, electing, ackldRecv, forwarding, tempMaxEpoch>>
           \/ /\ newState = LOOKING
...
FleUpdateUnchanged(i) ==
    /\ FleUnchanged

(* Fast Leader Election *)
FleZabTimeoutMod == FleZabTimeout(IsON, FleUpdateUnchanged)

FLEReceiveNotmsgMod ==
    FleReceiveNotmsg(IsON, FleUpdateUnchanged)

FLENotmsgTimeoutMod ==
    FleNotmsgTimeout(IsON, FleUpdateUnchanged)

FLEHandleNotmsgOrFLEWaitNewNotmsgMod ==
    FleHandleNotmsgOrWaitNewNotmsg(IsON, FleHanleMsg)
...
ZabTimeoutInCluster(i, S) ==
        ...
        /\ logicalClock' = [s \in Server |-> IF i = s THEN 0 ELSE
                                             IF s \in S THEN logicalClock[s]\* + 1 
                                                        ELSE logicalClock[s] ]
        ...
        \* /\ recvQueue' = [s \in Server |-> IF s \in S THEN << [mtype |-> NONE] >>  ELSE recvQueue[s] ]
        /\ recvQueue' = [s \in Server |-> IF s \in S THEN << [mtype |-> NONE] >>  ELSE recvQueue[s] ]
...
NodeCrash(i) ==
        ...
        /\ \/ /\ IsLooking(i)
              /\ logicalClock' = [logicalClock EXCEPT ![i] = 0]
              /\ UNCHANGED <<serverVarsL, leaderVarsL, historyVotes, currentVote, receiveVotes,
                             outOfElection, recvQueue, waitNotmsg,
                             zabState, connectInfo, learners, forwarding, connecting, electing, ackldRecv>>
           \/ /\ IsFollower(i)
        ...
        /\ LET crash1 == net1!NetDelConn({i})
               crash2 == net2!NetDelConn({i})
            \*    n_wire == ("n_wire" :> crash1[1].n_wire + crash2[1].n_wire - recorder.n_wire)
               n_wire == ("n_wire" :> crash1[1].n_wire + crash2[1].n_wire)
               init_set == {x \in Server: (status'[x] = ONLINE /\ recvQueue[x] /= recvQueue'[x] /\ Len(recvQueue'[x]) = 1 /\ recvQueue'[x][1].mtype = NONE)}
               lock == IF Cardinality(init_set) > 0 THEN INITING ELSE recorder.lock
               pc == <<"NodeCrash", i, IF lock = INITING THEN "LeaderShutdown" ELSE
                                       IF IsLooking(i) THEN "LookingShutdown" ELSE "FollowerShutdown">>
           IN /\ electionMsgs' = crash1[2]
              /\ msgs' = crash2[2]
              /\ recorder' = [lock |-> lock, init |-> init_set] @@ n_wire @@ RecorderIncCrash @@ UpdateRecorderZab(pc) @@ crash1[1] @@ recorder
              /\ netcmd' = <<pc, crash1[3]>>
...
\*         /\ UNCHANGED <<currentEpoch, zabState, acceptedEpoch, history,
\*                        lastSnapshot, initialHistory, followerVars, verifyVars, msgs,
\*                        ackldRecv, connecting, electing, forwarding, learners, tempMaxEpoch>>
\*         \* /\ recorder' = [lock |-> INITING, init |-> {i}] @@ recorder
\*         \* /\ netcmd' = <<"NodeStart", i>>
\*         /\ ZabFLEResetInit(i)
\*         /\ LET conn == net1!NetAddSingleNodeConn(i)
\*            IN net1!NetUpdate2(net1!NetArg(conn[1], ZabFLEBroadcastNoUpdate(i)), <<"NodeStart", i>>)
...
NodeStart(i) ==
        ...
        /\ net1!NetUpdate2(net1!NetArg([lock |-> INITING, init |-> {i}], net1!NetAddSingleNodeConn(i)), <<"NodeStart", i>>)

CrashAll ==
                /\ PreCheckCrash
                /\ UNCHANGED <<acceptedEpoch, lastCommitted, lastSnapshot, tempMaxEpoch,
                               initialHistory, verifyVars, packetsSync, currentEpoch, history>>
                /\ lastProcessed' = [s \in Server |-> InitLastProcessed(s) ]
                /\ state'         = [s \in Server |-> LOOKING]
                /\ currentVote'   = [s \in Server |-> SelfVote(s)]
                ...
                /\ ackldRecv'        = [s \in Server |-> {}]
                /\ forwarding'       = [s \in Server |-> {}]
                /\ connectInfo' = [s \in Server |-> [sid |-> NullPoint,
                                                        syncMode |-> NONE,
                                                        nlRcv |-> FALSE ] ]
                /\ electionMsgs' = [s \in Server |-> [v \in Server |-> << >>] ]
                /\ status'    = [s \in Server |-> ONLINE ]
                \* /\ recorder' = RecorderIncCrash @@ [recorder EXCEPT !.pc =  <<"RestartAll">> ]
                /\ msgs' = [s \in Server |-> [v \in Server \ {s} |-> << >>] ]
                /\ LET pc == <<"CrashAll">>
                   IN   /\ recorder' = RecorderIncCrash @@ [ lock |-> INITING, init |-> Server, n_wire |-> 0] @@ recorder
                        /\ netcmd' = <<pc>>
...
ConnectAndFollowerSendFOLLOWERINFO(i, j) ==
        ...
        /\ Net2ActionArg(Send(j, i, [ mtype |-> FOLLOWERINFO, mzxid |-> <<acceptedEpoch[j], 0>> ]),
                         <<"ConnectAndFollowerSendFOLLOWERINFO", i, j>>, RecorderClearZabLock)
...
LeaderProcessFOLLOWERINFO(i, j) ==
        ...
        /\ LET msg == msgs[j][i][1]
               ...
               pc == <<"LeaderProcessFOLLOWERINFO", i, j>>
           IN 
           /\ infoOk
                 ...
                 /\ LET new_sid_connecting == {c.sid: c \in connecting'[i]}
                    ...
                    \/ /\ WaitingForNewEpochTurnToFalse(i, new_sid_connecting)
                       ...
                       /\ LET newLeaderZxid == <<acceptedEpoch'[i], 0>>
                              m == [ mtype |-> LEADERINFO,
                                     mzxid |-> newLeaderZxid ]
                          IN Net2Action(DiscardAndBroadcastLEADERINFO(i, j, m), Append(pc, "broadcast"))
                    \/ /\ ~WaitingForNewEpochTurnToFalse(i, new_sid_connecting)
                       /\ UNCHANGED acceptedEpoch
                       /\ Net2Action(Discard(j, i), Append(pc, "discard"))
              \/  \* 2. has broadcast LEADERINFO 
                 /\ ~WaitingForNewEpoch(i, sid_connecting)
                 /\ UNCHANGED <<tempMaxEpoch, connecting, acceptedEpoch, violatedInvariants>>
                 /\ Net2Action(Reply(i, j, [ mtype |-> LEADERINFO, mzxid |-> <<acceptedEpoch[i], 0>> ]), Append(pc, "Reply"))
                          ...
                          /\ \/ /\ newEpoch > acceptedEpoch[i]
                                ...
                                /\ LET epochBytes == currentEpoch[i]
                                       m == [ mtype  |-> ACKEPOCH,
                                              ...
                                              mepoch |-> epochBytes ] 
                                   IN Net2Action(Reply(i, j, m), Append(pc, "newEpoch bigger"))
                             \/ /\ newEpoch = acceptedEpoch[i]
                                /\ UNCHANGED acceptedEpoch
                                /\ LET m == [ mtype  |-> ACKEPOCH,
                                              mzxid  |-> lastProcessed[i].zxid,
                                              mepoch |-> -1 ]
                                   IN Net2Action(Reply(i, j, m), Append(pc, "newEpoch equal"))
                       \/ /\ ~stateOk
                          /\ PrintT("Exception: Follower receives LEADERINFO," \o
                             " whileZabState not DISCOVERY.")
                          /\ violatedInvariants' = [violatedInvariants EXCEPT !.stateInconsistent = TRUE]
                          /\ UNCHANGED <<acceptedEpoch, zabState>>
                          /\ Net2Action(Discard(j, i), Append(pc, "state not ok"))
                 \/ \* 2. Abnormal case - go back to election
                    /\ ~epochOk
                    /\ FollowerShutdown(i)
                    /\ RemoveLearner(connectInfo[i].sid, i)
                    /\ UNCHANGED <<acceptedEpoch, violatedInvariants>>
                    /\ LET fleMsg == ZabFLEBroadcastNoUpdate(i)
                           cleanmsg == Clean(i)
                           n_wire == ("n_wire" :> fleMsg[1].n_wire - cleanmsg[1].n_wire)
                       IN /\ msgs' = cleanmsg[2]
                          /\ net1!NetUpdate2(net1!NetArg(n_wire, fleMsg),  Append(pc, "epoch not ok"))
...
SendSyncMsgs(i, j, lastSeenZxid, lastSeenIndex, mode, needRemoveHead) ==
        ...
        /\ LET lastCommittedIndex == IF zabState[i] = BROADCAST 
               ...
               pc == <<"LeaderSyncFollower", i, j, mode>>
               zabLockArgs == RecorderClearZabLock
           IN /\ proposalMsgsLog' = proposalMsgsLog \union set_forChecking
              /\ \/ /\ needRemoveHead
                    /\ Net2ActionArg(DiscardAndSendPackets(i, j, queue_toSend), pc, zabLockArgs)
                 \/ /\ ~needRemoveHead
                    /\ Net2ActionArg(SendPackets(i, j, queue_toSend), pc, zabLockArgs)
...
LeaderProcessACKEPOCH(i, j) ==
        ...
        /\ LET msg == msgs[j][i][1]
               ...
               pc == <<"LeaderProcessACKEPOCH", i, j>>
               zabLockArgs == [ zabLock |-> "sync", zabLockArgs |-> <<i, j>> ]
           IN /\ infoOk
              /\ \/ \* electionFinished = true, jump ouf of waitForEpochAck. 
                    ...
                    /\ Net2ActionArg(Discard(j, i), Append(pc, "ElectionFinished"), zabLockArgs)
                 \/ /\ ~ElectionFinished(i, sid_electing)
                    /\ \/ /\ zabState[i] = DISCOVERY
                          /\ UNCHANGED violatedInvariants
                       \/ /\ zabState[i] /= DISCOVERY
                    ...
                    /\ \/ /\ followerStateSummary.currentEpoch = -1
                          ...
                          /\ Net2ActionArg(Discard(j, i), Append(pc, "currentEpoch=-1"), zabLockArgs)
                       \/ /\ followerStateSummary.currentEpoch > -1
                          /\ \/ \* normal follower 
                                /\ logOk
                                /\ electing' = [electing EXCEPT ![i] = UpdateElecting(@, j, msg.mzxid, TRUE) ]
                             ...
                             \/ \* Exists follower more recent than leader
                                ...
                                /\ LET cleanMsgs == CleanConn({i})
                                       fleMsg == Head(cleanMsgs)
                                       init_set == {x \in Server: (status'[x] = ONLINE /\ recvQueue[x] /= recvQueue'[x] /\ Len(recvQueue'[x]) = 1 /\ recvQueue'[x][1].mtype = NONE)}
                                       lock == IF Cardinality(init_set) > 0 THEN INITING ELSE recorder.lock
                                       net2updater == <<[lock |-> lock, init |-> init_set] @@ cleanMsgs[2], cleanMsgs[3], cleanMsgs[4]>>
                                   IN /\ electionMsgs' = fleMsg
                                      /\ Net2Action(net2updater, Append(pc, "LeaderShutdown"))
...
LeaderSyncFollower(i, j) ==
        ...
        /\ LET electing_quorum == {e \in electing[i]: e.inQuorum = TRUE }
           ...
           /\ LET chosen == CHOOSE s \in toSync: s.sid = j
              ...
              IN /\ UNCHANGED <<state, currentEpoch, lastProcessed, zabState, acceptedEpoch, 
                 ...
                 /\ SyncFollower(i, chosen.sid, chosen.peerLastZxid, FALSE)
...
FollowerProcessSyncMessage(i, j) ==
        ...
        /\ LET msg == msgs[j][i][1]
           ...
           IN /\ infoOk
              ...
              /\ Net2ActionArg(Discard(j, i), <<"FollowerProcessSyncMessage", i, j, msg.mtype>>,
                               [ zabLock |-> "fsync", zabLockArgs |-> <<i, j>>])
...
FollowerProcessPROPOSALInSync(i, j) ==
        ...
        /\ Net2Action(Discard(j, i), <<"FollowerProcessPROPOSALInSync", i, j>>)
...
FollowerProcessCOMMITInSync(i, j) ==
        ...
        /\ LET msg == msgs[j][i][1]
           ...
           IN /\ infoOk 
              ...
              /\ Net2Action(Discard(j, i), <<"FollowerProcessCOMMITInSync", i, j, IF ~exist THEN "~exist" ELSE 
                                                                                  IF match THEN "match" ELSE "~match">>)
        ...
        /\ LET msg == msgs[j][i][1]
           ...
           IN /\ infoOk
              ...
              /\ UNCHANGED <<state, lastProcessed, zabState, acceptedEpoch, lastCommitted, 
                       ...
                       proposalMsgsLog, epochLeader, electionMsgs, envVars>>
              /\ Net2Action(DiscardAndSendPackets(i, j, queue_toSend), <<"FollowerProcessNEWLEADER", i, j, IF SnapshotNeeded(i) THEN "snapshot" ELSE "not-snapshot">>)
        ...
        /\ LET msg    == msgs[j][i][1]
           ...
           IN /\ infoOk
              /\ \/ \* just reply UPTODATE.
                    ...
                    /\ Net2Action(Reply(i, j, m_uptodate), pc)
                 \/ /\ ~QuorumFormed(i, sid_ackldRecv)
                    /\ \/ /\ match
                          ...
                          /\ LET new_sid_ackldRecv == {a.sid: a \in ackldRecv'[i]}
                             ...
                             \/ \* jump out of waitForNewLeaderAck, and do startZkServer,
                                ...
                                /\ LeaderTurnToBroadcast(i)
                                /\ Net2Action(DiscardAndBroadcastUPTODATE(i, j, m_uptodate), Append(pc, "QuorumFormed"))
                             \/ \* still wait in waitForNewLeaderAck.
                                /\ ~QuorumFormed(i, new_sid_ackldRecv)
                                /\ UNCHANGED <<zabState, lastCommitted, lastProcessed,
                                               lastSnapshot, currentVote>>
                                /\ Net2Action(Discard(j, i), Append(pc, "~QuorumFormed"))
                          /\ UNCHANGED violatedInvariants
                       \/ /\ ~match
                          ...
                          /\ Net2Action(Discard(j, i), Append(pc, "~match"))
...
FollowerLogRequestInBatches(i, leader, ms_ack, packetsNotCommitted) ==
        ...
        /\ Net2Action(DiscardAndSendPackets(i, leader, ms_ack), <<"FollowerProcessUPTODATE", i, leader>>)
...
LeaderProcessRequest(i) ==
        ...
        /\ LET request_value == GetRecorder("nClientRequest") \* unique value
           ...
           IN /\ history' = [history EXCEPT ![i] = Append(@, newTxn) ]
              ...
              /\ Net2ActionArg(Broadcast(i, m_proposal), <<"LeaderProcessRequest", i, IF ShouldSnapshot(i) THEN "snapshot" ELSE "not-snapshot">>, RecorderIncRequests)
...
FollowerProcessPROPOSAL(i, j) ==
        ...
        /\ LET msg == msgs[j][i][1]
               ...
               m_ack  == [ mtype |-> ACK,
                           mzxid |-> msg.mzxid ]
               pc == <<"FollowerProcessPROPOSAL", i, j>>
          IN /\ infoOk
             /\ \/ /\ isNext
                   ...
                   /\ \/ /\ ShouldSnapshot(i)
                      ...
                      \/ /\ ~ShouldSnapshot(i)
                         /\ UNCHANGED <<lastSnapshot, violatedInvariants>>
                   /\ Net2Action(Reply(i, j, m_ack), Append(pc, IF ShouldSnapshot(i) THEN "snapshot" ELSE "not-snapshot"))
                \/ /\ ~isNext
...
LeaderCommit(s, follower, index, zxid) ==
        ...
        /\ LET m_commit == [ mtype |-> COMMIT,
                             mzxid |-> zxid ]
           IN Net2Action(DiscardAndBroadcast(s, follower, m_commit), <<"LeaderProcessACK", s, follower, "commit">>)
...
LeaderTryToCommit(s, index, zxid, newTxn, follower) ==
        ...
        IN \/ /\ \* Current conditions do not satisfy committing the proposal.
              ...
              /\ Net2Action(Discard(follower, s), <<"LeaderProcessACK", s, follower, "not-commit">>)
           \/ /\ allTxnsBeforeCommitted
              /\ hasAllQuorums
              /\ \/ /\ ~ordered
              ...
              /\ lastProcessed' = [lastProcessed EXCEPT ![s] = [ index |-> index,
                                                                 zxid  |-> zxid ] ]
              /\ LeaderCommit(s, follower, index, zxid)
            \*   /\ Net2Action(Discard(follower, s), <<"LeaderProcessACK", s, follower>>)
              ...
              /\ \/ /\ exist
                    ...
                    /\ LET txn == history[i][index]
                       ...
                       /\ \/ /\ \* Note: outstanding is 0. 
                                ...
                                \/ ~outstanding
                                \/ hasCommitted
                             /\ Net2Action(Discard(j, i), <<"LeaderProcessACK", i, j, "hasCommitted">>)
                             /\ UNCHANGED <<violatedInvariants, lastCommitted, lastProcessed>>
                          \/ /\ outstanding
                             /\ ~hasCommitted
                 ...
                 \/ /\ \/ ~exist
                    ...
                    /\ UNCHANGED <<history, lastCommitted, lastProcessed>>
                    /\ Net2Action(Discard(j, i), <<"LeaderProcessACK", i, j, "Exception">>)
...
FollowerProcessCOMMIT(i, j) ==
        ...
        /\ LET msg == msgs[j][i][1]
           ...
           /\ Net2Action(Discard(j, i), <<"FollowerProcessCOMMIT", i, j, IF noPending THEN "noPending" ELSE
                                                                         IF ZxidEqual(pendingTxns[1].zxid, msg.mzxid) THEN "match" ELSE "~match">>)
-----------------------------------------------------------------------------
...
FilterNonexistentMessage(i) ==
        /\ \E j \in Server \ {i}: /\ msgs[j][i] /= << >>
                                  ...
                                  /\ Net2Action(Discard(j, i), <<"FilterNonexistentMessage", i, j>>)
        \* /\ UnchangeRecorder
...
IncCurrEpoch ==
    \A i \in Server: currentEpoch[i] = 0
...
Do2(func(_, _)) ==
    \E i, j \in Server: i /= j /\ IsNotFLE /\ func(i, j) /\ historyVotes' = UpdateHistVote

Do1(func(_)) == 
    \E i \in Server: IsNotFLE /\ func(i) /\ historyVotes' = UpdateHistVote

Next == 
    \/  /\ recorder.zabLock = NONE
        (* FLE module *)
            \* \/ \E i, j \in Server: FLEReceiveNotmsg(i, j)/\ historyVotes' = UpdateHistVote
            \* \/ \E i \in Server:    FLENotmsgTimeout(i)/\ historyVotes' = UpdateHistVote
            \* \/ \E i \in Server:    FLEHandleNotmsg(i)
            \* \/ \E i \in Server:    FLEWaitNewNotmsg(i)/\ historyVotes' = UpdateHistVote
        /\  \/ FleZabTimeoutMod
            \/ FLEReceiveNotmsgMod
            \/ FLENotmsgTimeoutMod
            \/ FLEHandleNotmsgOrFLEWaitNewNotmsgMod
        (* situation errors like failure, network partition *)
            \* \/ \E i, j \in Server: PartitionStart(i, j)/\ historyVotes' = [historyVotes EXCEPT ![i] = <<TRUE>>]
            \* \/ \E i, j \in Server: PartitionRecover(i, j)/\ historyVotes' = UpdateHistVote
            \* \/ \E i \in Server:    IsNotFLE /\ NodeCrash(i) /\ historyVotes' = [historyVotes EXCEPT ![i] = <<TRUE>>]
            \* \/ \E i \in Server:    IsNotFLE /\ NodeStart(i)/\ historyVotes' = UpdateHistVote
            \* \/ IsNotFLE /\ CrashAll
            \/ \E i \in Server:              IsNotFLE /\ NodeCrash(i)                               /\ historyVotes' = [historyVotes EXCEPT ![i] = <<TRUE>>]
            \* \/ \E i \in Server:              IsNotFLE /\ NodeStart(i)                               /\ historyVotes' = UpdateHistVote
            \/ Do1(NodeStart)
        (* Zab module - Discovery and Synchronization part *)
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ ConnectAndFollowerSendFOLLOWERINFO(i, j)   /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ LeaderProcessFOLLOWERINFO(i, j)            /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessLEADERINFO(i, j)            /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ LeaderProcessACKEPOCH(i, j)                /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ LeaderSyncFollower(i, j)                   /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessSyncMessage(i, j)           /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessPROPOSALInSync(i, j)        /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessCOMMITInSync(i, j)          /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessNEWLEADER(i, j)             /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ LeaderProcessACKLD(i, j)                   /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessUPTODATE(i, j)              /\ historyVotes' = UpdateHistVote
            \/ Do2(ConnectAndFollowerSendFOLLOWERINFO)
            \/ Do2(LeaderProcessFOLLOWERINFO)
            \/ Do2(FollowerProcessLEADERINFO)
            \/ Do2(LeaderProcessACKEPOCH)
            \/ Do2(LeaderSyncFollower)
            \/ Do2(FollowerProcessSyncMessage)
            \/ Do2(FollowerProcessPROPOSALInSync)
            \/ Do2(FollowerProcessCOMMITInSync)
            \/ Do2(FollowerProcessNEWLEADER)
            \/ Do2(LeaderProcessACKLD)
            \/ Do2(FollowerProcessUPTODATE)
        (* Zab module - Broadcast part *)
            \* \/ \E i \in Server:              IsNotFLE /\ LeaderProcessRequest(i)                    /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessPROPOSAL(i, j)              /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ LeaderProcessACK(i, j)                     /\ historyVotes' = UpdateHistVote
            \* \/ \E i, j \in Server: i /= j /\ IsNotFLE /\ FollowerProcessCOMMIT(i, j)                /\ historyVotes' = UpdateHistVote
            \/ Do1(LeaderProcessRequest)
            \/ Do2(FollowerProcessPROPOSAL)
            \/ Do2(LeaderProcessACK)
            \/ Do2(FollowerProcessCOMMIT)
        (* An action used to judge whether there are redundant messages in network *)
            \* \/ \E i \in Server:              IsNotFLE /\ FilterNonexistentMessage(i)                /\ historyVotes' = UpdateHistVote
            \/ Do1(FilterNonexistentMessage)
    \/  /\ recorder.zabLock /= NONE
        /\ IF recorder.zabLock = "sync"  THEN LeaderSyncFollower(recorder.zabLockArgs[1], recorder.zabLockArgs[2]) ELSE 
           IF recorder.zabLock = "fsync" THEN
                \/ FollowerProcessSyncMessage(recorder.zabLockArgs[1], recorder.zabLockArgs[2])
                \/ FollowerProcessPROPOSALInSync(recorder.zabLockArgs[1], recorder.zabLockArgs[2])
                \/ FollowerProcessCOMMITInSync(recorder.zabLockArgs[1], recorder.zabLockArgs[2])
                \/ FollowerProcessNEWLEADER(recorder.zabLockArgs[1], recorder.zabLockArgs[2]) 
           ELSE
           IF recorder.zabLock = "fconn" THEN
                \/ /\ IsON(recorder.zabLockArgs[1])
                   /\ ConnectAndFollowerSendFOLLOWERINFO(recorder.zabLockArgs[1], recorder.zabLockArgs[2])
                \/ /\ ~IsON(recorder.zabLockArgs[1])
                   /\ recorder' = RecorderClearZabLock @@ recorder
                   /\ netcmd' = <<<<"ClearLock", recorder.zabLockArgs[1], recorder.zabLockArgs[2], "fconn">>>>
                   /\ UNCHANGED <<serverVars, electionVars, leadingVoteSet, connecting, learners, connectInfo,
                                  electing, ackldRecv, forwarding, tempMaxEpoch,
                                  verifyVars, envVars, electionMsgs, packetsSync, msgs>>
           ELSE
