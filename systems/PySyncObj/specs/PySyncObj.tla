...
RECURSIVE _GetAppendEntriesNoSend(_, _, _, _)
_GetAppendEntriesNoSend(n, preNextIndex, res, left) ==
    IF Len(left) = 0 THEN res
    ELSE LET s == Head(left)
             remaining == Tail(left)
             nextNodeIdx == preNextIndex[n][s]
             curLog == log'[n]
             m ==  [ src |-> n, dst |-> s, type |-> M_AE,
                                data |-> [ term |-> currentTerm'[n],
                                           ...
                                           prevLogIdx |-> GetPrevLogIdx(curLog, nextNodeIdx),
                                           prevLogTerm |-> GetPrevLogTerm(curLog, nextNodeIdx)]]
         IN _GetAppendEntriesNoSend(n, preNextIndex, Append(res, m), remaining)

GetAppendEntriesNoSend(n, preNextIndex) == _GetAppendEntriesNoSend(n, preNextIndex, <<>>, SetToSeq(Servers \ {n}))
...
\*                           [ j \in Servers \ {dst} |-> 0 ]]
                       ...
                       /\ LET preNextIndex == [ nextIndex' EXCEPT ![dst] = [ j \in Servers |-> @[j] - 1 ]]
                          IN NetUpdate2(NetReplyBatchAddMsg(GetAppendEntriesNoSend(dst, preNextIndex), m),
                  ...
                  ELSE IF prevEntries[1].term /= prevLogTerm  \* log mismatch (term not equal)
                       ...
                       ELSE \* CHANGED: log, commitIndex
                            /\ LET afterDeleteLog == DeleteEntriesFrom(dst, prevLogIdx + 1)
                                   ...
                                   nextNodeIdx    == IF Len(entries) = 0 THEN prevLogIdx + 1 ELSE entries[Len(entries)].idx + 1
...
HandleMsgNNI(m) ==
    ...
    IN /\ IF CheckStateIs(dst, Leader)
          THEN /\ IF reset = TRUE THEN nextIndex' = [ nextIndex EXCEPT ![dst][src] = nextNodeIdx ]
                  ELSE IF success = TRUE THEN TRUE ELSE UNCHANGED nextIndex
               /\ IF success = TRUE
                  THEN IF currentNodeIdx > matchIndex[dst][src]
                       THEN /\ matchIndex' = [ matchIndex EXCEPT ![dst][src] = currentNodeIdx ]
                            /\ nextIndex' = [ nextIndex EXCEPT ![dst][src] = nextNodeIdx ]
        ...
        ELSE LET entries == SubSeq(log'[i], 1, commitIndex'[i])
                 ...
                 F[j \in 0..nServer] ==
                    ...
                    ELSE LET k == CHOOSE k \in Servers: k \notin F[j-1][1]
                             logLenOk == log'[k][Len(log'[k])].idx >= commitIndex'[i]
...
FollowerLogLELeaderLogAfterAE ==
    LET cmd  == netcmd'[1]
        ...
        follower == cmd[3]
        leader   == cmd[4]
    IN IF cmd1 = "HandleMsgAE" /\ cmd2 \in { "success", "no-prevEntries" }
       THEN IF log[follower] /= log'[follower]
            THEN log'[follower][Len(log'[follower])].idx <= log'[leader][Len(log'[leader])].idx
            ELSE TRUE
       ELSE TRUE

CommitIdxLELogLen ==
    \A i \in Servers: commitIndex'[i] <= log'[i][Len(log'[i])].idx
...
DoClientRequest ==
    ...
    /\ \E n \in Servers:
        /\ CheckStateIs(n, Leader)
        /\ \E v \in Commands: ClientRequest(n, v)
    /\ inv' = InvSequence
...
\* Last modified Sun Nov 27 18:24:01 CST 2022 by tangruize
