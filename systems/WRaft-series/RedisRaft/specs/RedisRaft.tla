...
RecvRequestVote(m) ==  \* func: raft_recv_requestvote
    LET req == m.body
        src == m.src
        dst == m.dst
        rejectHasLeaderId == /\ req.prevote
                             /\ leader_id[dst] /= Nil
                             /\ leader_id[dst] /= req.candidate_id
                             /\ state[dst] = Leader
        demote == ~rejectHasLeaderId /\ ~req.prevote /\ current_term[dst] < req.term
...
MonotonicCurrentTerm == current_term /= current_term' => \A i \in Servers: current_term' [i] >= current_term[i]

MonotonicCommitIdx == commit_idx /= commit_idx' => \A i \in Servers: commit_idx'[i] >= commit_idx[i]
...
NextIdxGtMatchIdx ==
  match_idx /= match_idx' \/ next_idx /= next_idx' =>
    \A i \in Servers:
        IF state'[i] = Leader
...
CommitIdxLELogLen ==
  commit_idx /= commit_idx' \/ log /= log' =>
    \A i \in Servers: commit_idx'[i] <= LogCount(log'[i])

LeaderCommitCurrentTermLogs ==
  commit_idx /= commit_idx' \/ current_term /= current_term' \/ state' /= state =>
    \A i \in Servers:
        IF state'[i] = Leader
...
NewLeaderTermNotInLog ==
  log /= log' \/ current_term /= current_term' =>
    \A i \in Servers:
        IF state'[i] = Leader /\ state[i] /= Leader
...
LeaderTermLogHasGreatestIdx ==
  log /= log' \/ current_term /= current_term' \/ state /= state' =>
    \A i \in Servers:
        IF state'[i] = Leader
        THEN \A j \in Servers \ {i}:
...
\* Last modified Fri Mar 31 12:20:57 CST 2023 by tangruize
