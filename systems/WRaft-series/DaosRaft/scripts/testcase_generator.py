...
def yield_trace(states):
    ...
    def get_converted_model_value(n, model_var_name):
        ...
        elif model_var_name in {'next_idx', 'match_idx'}:
            if n in model_value:
                model_value.pop(n)
    ...
    def do_tick(n, is_compare=True):
        ...
        if is_compare:
            ...
            if model_value == "4":  # leader
                yield from compare(n, 'next_idx', 'next_idx')
                yield from compare(n, 'match_idx', 'match_idx')
            yield from compare(n, 'commit_idx', 'commit_idx')
            yield from compare(n, 'current_term', 'current_term')
                ...
                for n in partitioned_nodes:
                    ...
                    for _ in range(2):
                        for j in nodes:
                            ...
                            yield from do_tick(j, is_compare=False)
                    yield ['wait-recover']
                    for j in nodes:
